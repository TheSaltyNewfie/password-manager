...
# Internal Functions
substitute = lambda state:[[s_box[state[q][w]] for w in range(4)] for q in range(4)]
substitute_i = lambda state:[[inv_s_box[state[q][w]] for w in range(4)] for q in range(4)]
...
xor = lambda x, y:[a ^ b for a, b in zip(x, y)]

xor_2d = lambda x, y:[xor(a, b) for a, b in zip(x, y)]

f = lambda a: ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else a << 1

mix_columns = lambda state:[[row[(q + 1) % 4] ^ row[(q + 2) % 4] ^ row[(q + 3) % 4] ^ f(row[q] ^ row[(q + 1) % 4]) for q in range(4)] for row in state]
mix_columns_i = lambda state:mix_columns([[row[q] ^ (f(f(row[1] ^ row[3])) if q % 2 else f(f(row[0] ^ row[2]))) for q in range(4)] for row in state])

# Helper Functions
bytes2matrix = lambda text:[[*text[q:(q + 4)]] for q in range(0, len(text), 4)]
matrix2bytes = lambda matrix:bytes(sum(matrix, []))

def split_blocks(message, block_size = 16):
    for q in range(0, len(message), block_size):
...
class AES:
    ...
    # Encrypt vis Cipher Block Chaining
    def encrypt_cbc(self, plaintext, init_vec):
        plaintext = pad(plaintext)

        assert len(init_vec) == 16

        blocks = []
        prev = init_vec
        for plaintext_block in split_blocks(plaintext):
            blocks += [self.encrypt_block(bytes(xor(plaintext_block, prev)))]
            prev = blocks[-1]
    ...
    def decrypt_cbc(self, ciphertext, init_vec):
        ...
        for ciphertext_block in split_blocks(ciphertext):
            blocks.append(bytes(xor(prev, self.decrypt_block(ciphertext_block))))
