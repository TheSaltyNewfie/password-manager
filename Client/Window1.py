...
def login():
    ...
    if "error" in response:
        ...
        return
    token = response["token"]
    response = api.get_passwords(User_Box.value, Pass_Box.value, token)
    if "error" in response:
        app.warn("Error", response["error"])
        return
    passwords = response
...
Master_Pass = Text(app, text="Master Password: ", grid=[0, 1], size=20, align="left", bg="white")
Pass_Box = TextBox(app, align="left", grid=[1, 1], width="fill", hide_text=True)
Pass_Box.bg = "DarkOrange1"

Login = PushButton(app, text="Login", grid=[0, 2], align="left", width=5, height=2, command=login)
...
Welcome = Text(app, text="New to Company? Welcome, sign up below:", grid=[0, 6], align="left", size=15, font= "Comic Sans MS")
Sign_Up = PushButton(app, text="Sign Up", grid=[0, 8], width=5, height=2, align="left", command=open_window)


##  Window 2  ##

New_User = Text(sign_up_window, text="User ID: ", size=20, grid=[0, 0], align="left", bg='white')
New_UserBox = TextBox(sign_up_window, grid=[1, 0], align="left", width="fill")
New_User.text_color = "red3"
New_UserBox.bg = "coral2"

New_Pass = Text(sign_up_window, text="Master Password: ", grid=[0, 1], size=20, align="left", bg="white")
...
Confirm_Pass = Text(sign_up_window, text="Confirm Password: ", grid=[0, 3], size=20, align="left", bg="white")
...
def show_passwords():
    ...
    for passwd in passwords[0]:
        Text(List_of_passwords, f"Title: {passwd['title']}, Username: {passwd['username']}, Password: {passwd['password']}")
...
def save_password():
    ...
    response = api.new_password(username, password, token, Username_TextBox.value, Password_TextBox.value)
    if "error" in response:
        app.warn("Error", response["error"])
        return
    app.info("Success!!!!!!!!!!", "You've successfully created a new password!")
    response = api.get_passwords(User_Box.value, Pass_Box.value, token)
    if "error" in response:
        app.warn("Error", response["error"])
        return
...
Save_Button = PushButton(Password_window, text="Save", height=1, width=2, grid=[1, 8], command=save_password)
